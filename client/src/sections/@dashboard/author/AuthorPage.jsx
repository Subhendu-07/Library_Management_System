...
const AuthorPage = () => {
  ...
  const addAuthor = () => {
    ...
    if (author.photo) formData.append("photo", author.photo);

    axios
      .post(apiUrl(routes.AUTHOR, methods.POST), formData, {
  ...
  const updateAuthor = () => {
    ...
    axios
      .put(apiUrl(routes.AUTHOR, methods.PUT, selectedAuthorId), formData, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then((response) => {
  ...
  const getSelectedAuthorDetails = () => {
    const selectedAuthor = authors.find(
      (element) => element._id === selectedAuthorId
    );
    setAuthor({
      ...selectedAuthor,
      photo: null, // reset file for update
    });
  };

  const clearForm = () => {
    setAuthor({ id: "", name: "", description: "", photoUrl: "", photo: null });
  };
                            ...
                            <TableRow hover key={_id} tabIndex={-1}>
                              <TableCell align="center">
                                <Stack direction="row" alignItems="center" spacing={4}>
                                   <Avatar alt={name} src={`http://localhost:8080${photoUrl}`} />
                                </Stack>
                              </TableCell>
